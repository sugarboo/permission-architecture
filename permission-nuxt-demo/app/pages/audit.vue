...
const { data: audit, refresh, pending } = await useAsyncData('audit', () => $fetch<any[]>('/api/audit'))
const search = ref('')
...
<template>
  <div>
    ...
    <section class="panel">
      <div class="panel-head"><div><h2>审计事件</h2><p>最近 {{ audit?.length || 0 }} 条权限中心事件。</p></div><UBadge color="success" variant="subtle" label="不可由前端伪造" /></div>
