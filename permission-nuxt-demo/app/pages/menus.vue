...
const { data: catalog, refresh, pending } = await useAsyncData('menus-catalog', () => $fetch<any>('/api/catalog'))
...
const form = reactive({ nodeType: 'MENU' as 'BOARD' | 'DIRECTORY' | 'MENU', name: '', code: '', parentId: undefined as string | undefined, permissionCode: '', routePath: '', componentKey: '', icon: 'i-lucide-panel-top', sortOrder: 100, status: 'ACTIVE' as 'ACTIVE' | 'DISABLED' })
...
const bindingCandidates = computed(() => (catalog.value?.functions || []).filter((item: any) => item.status === 'ACTIVE' && `${item.name} ${item.code}`.toLowerCase().includes(bindingSearch.value.toLowerCase())))
...
watch(() => form.nodeType, type => { form.parentId = undefined; if (type !== 'MENU') { form.permissionCode = ''; form.routePath = ''; form.componentKey = '' } })
...
function resetMenu() { Object.assign(form, { nodeType: 'MENU', name: '', code: '', parentId: undefined, permissionCode: '', routePath: '', componentKey: '', icon: 'i-lucide-panel-top', sortOrder: 100, status: 'ACTIVE' }) }

async function createMenu() {
  saving.value = true
  try {
    const result = await $fetch<any>('/api/menus', { method: 'POST', body: form })
...
async function publishBindings() {
  ...
  saving.value = true
  try {
    await $fetch(`/api/menus/${selectedId.value}/bindings`, { method: 'PUT', body: { corePermissionIds, optionalPermissionIds, reason: bindingReason.value } })
...
<template>
  <div>
    <PageHeader title="板块 → 目录 → 页面菜单" description="板块与目录只用于分组和批量选择当前叶子；只有页面菜单持久化授权。授予页面菜单时自动展开 CORE，高风险操作保持 OPTIONAL。"><UButton color="primary" icon="i-lucide-panel-top-open" label="新建菜单节点" @click="createOpen = true" /></PageHeader>
    <div class="split-grid">
      ...
      <section class="panel">
        <template v-if="selected"><div class="panel-head"><div><h2>{{ selected.name }}</h2><p class="code">{{ selected.code }}</p></div><UButton v-if="selected.nodeType === 'MENU'" color="primary" variant="soft" icon="i-lucide-package-open" label="编辑权限包" @click="openBindings" /></div>
          <div class="panel-body"><div class="detail-hero"><div style="display: flex; justify-content: space-between; gap: 12px"><div><UBadge color="primary" variant="subtle" :label="selected.nodeType" /><h3 style="margin-top: 10px">{{ selected.name }}</h3><p>{{ selected.nodeType === 'MENU' ? '这是可授权的页面菜单叶子。CORE 随菜单展开，OPTIONAL 由管理员单独选择。' : '这是结构分组节点；数据库不会保存未来后代通配授权。' }}</p></div><UIcon :name="selected.nodeType === 'BOARD' ? 'i-lucide-layout-grid' : selected.nodeType === 'DIRECTORY' ? 'i-lucide-folder-tree' : 'i-lucide-panel-top'" size="32" style="color: #246bfd" /></div>
            <div class="detail-grid"><div class="detail-cell"><span>上级</span><b>{{ allMenus.find(item => item.id === selected?.parentId)?.name || '根节点' }}</b></div><div class="detail-cell"><span>排序</span><b>{{ selected.sortOrder }}</b></div><div class="detail-cell"><span>状态</span><b>{{ selected.status }}</b></div><div class="detail-cell"><span>前端路由</span><b class="code">{{ selected.routePath || '—' }}</b></div><div class="detail-cell"><span>组件键</span><b class="code">{{ selected.componentKey || '—' }}</b></div><div class="detail-cell"><span>授权叶子</span><b class="code">{{ selected.permissionId || '无' }}</b></div></div></div>
            <div v-if="selected.nodeType === 'MENU'" class="panel" style="margin-top: 16px; box-shadow: none"><div class="panel-head"><div><h3>当前已发布权限包</h3><p>{{ selected.coreCount || 0 }} CORE · {{ selected.optionalCount || 0 }} OPTIONAL</p></div></div><div class="panel-body source-list"><div v-for="item in currentBindings" :key="item.permissionId" class="source-card"><div style="display: flex; justify-content: space-between; gap: 10px"><b>{{ item.name }}</b><UBadge :color="item.bundleLevel === 'CORE' ? 'primary' : 'neutral'" variant="subtle" :label="item.bundleLevel" /></div><p class="code">{{ item.code }} · {{ item.type }} · {{ item.riskLevel }}</p></div><EmptyState v-if="!currentBindings.length" title="尚未绑定权限包" description="页面菜单可以先创建，再发布至少一项安全的 CORE 功能。"><UButton color="primary" size="sm" label="配置权限包" @click="openBindings" /></EmptyState></div></div>
    ...
    <USlideover v-model:open="bindingOpen" :title="`发布权限包 · ${selected?.name || ''}`" description="CORE 自动随菜单授予；OPTIONAL 由用户或角色单独选择。" :dismissible="!saving" :ui="{ content: 'sm:max-w-2xl' }">
      <template #body><div class="toolbar"><UInput v-model="bindingSearch" class="search-box" icon="i-lucide-search" placeholder="搜索业务功能" /><div class="tags"><UBadge color="primary" variant="subtle" :label="`${coreCount} CORE`" /><UBadge color="neutral" variant="subtle" :label="`${optionalCount} OPTIONAL`" /></div></div><div class="source-list"><div v-for="item in bindingCandidates" :key="item.id" class="source-card"><div style="display: flex; gap: 12px; align-items: center"><div style="min-width: 0; flex: 1"><b>{{ item.name }}</b><p class="code">{{ item.code }} · FUNCTION · {{ item.riskLevel }}</p></div><div class="tab-strip"><button class="tab-button" :aria-label="`${item.name} 不绑定`" :class="{ active: bindingLevels[item.id] === 'NONE' || !bindingLevels[item.id] }" @click="setLevel(item, 'NONE')">不绑定</button><button class="tab-button" :aria-label="`${item.name} CORE`" :disabled="item.riskLevel === 'HIGH'" :class="{ active: bindingLevels[item.id] === 'CORE' }" @click="setLevel(item, 'CORE')">CORE</button><button class="tab-button" :aria-label="`${item.name} OPTIONAL`" :class="{ active: bindingLevels[item.id] === 'OPTIONAL' }" @click="setLevel(item, 'OPTIONAL')">OPTIONAL</button></div></div></div></div><div class="inline-alert warning" style="margin-top: 14px"><UIcon name="i-lucide-triangle-alert" size="16" /><span>菜单权限包只绑定业务功能；高风险功能永远不能设为 CORE。发布会生成 before / after 审计，并刷新受影响策略版本。</span></div><UFormField label="变更原因" required style="margin-top: 14px"><UTextarea v-model="bindingReason" autoresize placeholder="说明本次菜单权限包变更的业务原因" /></UFormField></template>
