...
const { data: roles, refresh, pending } = await useAsyncData('roles', () => $fetch<any[]>('/api/roles'))
const search = ref('')
const createOpen = ref(false)
const saving = ref(false)
const authOpen = ref(false)
const authTarget = reactive({ id: '', name: '' })
const form = reactive({ name: '', code: '', domain: '信息板块', description: '', status: 'ACTIVE' as 'ACTIVE' | 'DISABLED' })
...
async function createRole() {
  saving.value = true
  try {
    await $fetch('/api/roles', { method: 'POST', body: form })
    showSuccess('角色模板已创建', '新角色默认没有任何权限，可继续进入树形授权。')
    createOpen.value = false
    Object.assign(form, { name: '', code: '', domain: '信息板块', description: '', status: 'ACTIVE' })
    await refresh()
...
<template>
  <div>
    <PageHeader title="角色是模板，不是边界" description="角色承担常规菜单与功能权限的批量复用；组织与岗位负责数据上下级，避免角色意外成为数据超管。"><UButton color="primary" icon="i-lucide-shield-plus" label="新建角色" @click="createOpen = true" /></PageHeader>
    <section class="panel">
      ...
      <div class="panel-body">
        ...
        <div class="table-scroll"><table class="data-table"><thead><tr><th>角色</th><th>适用板块</th><th>成员</th><th>菜单</th><th>显式功能</th><th>状态</th><th>操作</th></tr></thead><tbody>
          <tr v-for="role in filtered" :key="role.id"><td><div class="primary-cell"><div class="row-avatar"><UIcon name="i-lucide-shield-check" size="16" /></div><div><b>{{ role.name }}</b><small class="code">{{ role.code }}</small></div></div></td><td>{{ role.domain }}</td><td>{{ role.memberCount }} 人</td><td>{{ role.menuCount || 0 }}</td><td>{{ role.functionCount || 0 }}</td><td><UBadge :color="role.status === 'ACTIVE' ? 'success' : 'neutral'" variant="subtle" :label="role.status === 'ACTIVE' ? '启用' : '停用'" /></td><td><div class="row-actions"><UButton color="primary" variant="ghost" size="sm" icon="i-lucide-git-branch-plus" label="授权" @click="openAuth(role)" /></div></td></tr>
        </tbody></table></div>
      </div>
    </section>
    ...
    <UModal v-model:open="createOpen" title="新建角色模板" description="编码创建后应保持稳定；数据范围不在角色中配置。" :dismissible="!saving" :ui="{ content: 'sm:max-w-xl' }">
      <template #body><div class="form-grid">
        ...
        <UFormField label="说明" class="span-2"><UTextarea v-model="form.description" autoresize placeholder="描述角色的业务使用边界" /></UFormField>
      </div><div class="inline-alert" style="margin-top: 14px"><UIcon name="i-lucide-shield" size="16" /><span>创建后权限为空，避免角色名称暗示自动授权；请通过树形授权显式选择当前菜单叶子与功能。</span></div></template>
