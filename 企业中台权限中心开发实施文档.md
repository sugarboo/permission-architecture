...
- 角色一经勾选，其菜单、显式资源与菜单 CORE 实时反显；OPTIONAL 黄色高亮并区分已满足和待选；
- 组织树、岗位列表与用户任职；
...
- 工号只在组织与岗位页面设置任职时选填；
- 用户授权时角色支持多选，并优先展示在授权抽屉上方；角色反显为只读投影，不写入用户直授；
...
- 角色扁平、可复用，不设置板块归属字段；
- 一个用户可以选择多个角色；
- 授权抽屉先显示角色多选，再显示菜单和资源；勾选角色后即时计算菜单、显式资源、菜单 CORE 与 OPTIONAL 状态；
- 用户直授只允许加授，不提供 DENY；
- 用户直授原因必填，去除空白后非空即可；
- 失效时间选填，留空代表长期有效；
- 常规权限应进入角色，直授应定期复核。
...
角色为多选，UI 应把角色模板放在菜单和资源之前。勾选角色后的菜单、资源与 CORE 反显只用于预览，提交体仍只包含 `roleIds` 和用户明确选择的 `directMenuIds` / `directResourceIds`，禁止把角色或 CORE 派生项复制为用户直授。`validTo` 非必填；若填写必须晚于当前时间。原因只要求 trim 后非空，不设置任意最短字符数。
...
1. 角色模板多选，显示已选数量和角色摘要；每次选择均重新计算完整投影；
2. 菜单页签，角色菜单只读反显，用户直接菜单仍可编辑；
3. 资源页签，角色显式资源和 CORE 自动反显；OPTIONAL 黄色高亮并区分“角色已授 / 用户已授 / 待选”；
4. 有效权限页签，按来源解释最终结果；
5. 授权原因、来源工单和可选失效时间；
6. 保存前展示新增、移除和间接变化摘要。
...
- [ ] 任职时工号可选；
- [ ] 用户授权支持多选角色且角色优先展示；
- [ ] 角色菜单与 CORE 实时反显且不污染用户直授，OPTIONAL 黄色区分已满足与待选；
- [ ] 资源码不是 API URL；
- [ ] 编码前缀统一；
- [ ] 前后端使用同一资源码；
- [ ] 后端对每个业务执行点做资源裁决；
