...
- **资源**表达少量可复用的受控资产或技术能力；
- **API/服务方法**只是后端执行点，不再作为运营人员直接分配的权限；
- **组织与岗位**决定用户能操作哪些业务数据；
...
3. 角色与用户都可以获得菜单、功能、资源权限；用户直授第一版只允许 **ALLOW**，不提供 DENY。
...
10. 现有 API 权限资产不丢弃：迁移期进入“接口注册表”，用于映射、覆盖检查和审计，但不再暴露为运营授权语言。
...
| 资源权限 | 资源 | 报表、模板、任务、集成操作等受控资源；高级授权入口 |
...
| 资源 Resource | “能访问什么”的受控资产/技术能力 | 是，但默认高级入口 | 研发登记，管理员少量分配 |
...
资源不是业务数据行，也不等同于 URL。典型示例：

```text
res.crm.customer.export_template   客户导出模板
res.crm.report.sales_overview      销售总览报表
res.sc.supplier.sync_trigger       供应商同步任务
res.designer.asset_library         公共素材库
```

普通权限管理员默认只在“高级”页签看到资源。资源直授用于迁移兼容、报表、任务、模板或非常细的技术例外，不应重新成为日常授权主语言。
...
执行点登记在接口注册表，并绑定所需功能/资源；它不是角色或用户的直接授权对象。URL 改名只更新执行点元数据，不改变角色与用户授权。
...
2. 系统计算新增、删除功能/资源、风险等级与受影响角色/用户；
3. 高风险新增要求二次确认并填写原因；
...
```text
CanInvoke(u, endpoint) = endpoint 已启用
                      AND endpoint 所需权限满足 anyOf/allOf 规则
                      AND 所需功能/资源 ∈ EffectivePermissions(u)

CanOperate(u, action, row) = CanInvoke(u, action.endpoint)
                          AND row 满足 action.dataDomain 的数据谓词
```

管理端 API 命名空间采用默认拒绝：未标记为公开、未登记或未绑定权限的接口不放行。复杂权限表达式应通过拆分语义单一的功能/接口解决，而不是在注解中堆叠难以测试的逻辑。
...
erDiagram
  ...
  IAM_ENDPOINT ||--o{ IAM_ENDPOINT_PERMISSION : requires
  IAM_PERMISSION ||--o{ IAM_ENDPOINT_PERMISSION : enforced
...
`FUNCTION/RESOURCE/MENU` 共享授权关系，但各自扩展信息放在专门表中。运营界面按类型分层展示。
...
iam_resource
  permission_id, resource_type, metadata_json
  # resource_type: REPORT | TEMPLATE | TASK | INTEGRATION | ASSET 等
...
iam_menu_binding
  menu_id
  bound_permission_id  # FUNCTION 或 RESOURCE
...
```text
iam_endpoint
  id, service_code, http_method, path_template
  handler_signature, status, sync_source, last_seen_version
  UNIQUE(service_code, http_method, path_template)

iam_endpoint_permission
  endpoint_id, permission_id, match_mode
  # match_mode 用于少量 ANY/ALL；默认 REQUIRED
```

HTTP 方法+路径在这里用于研发覆盖检查，不进入用户/角色授权表。
...
- 禁止物理删除仍被角色、菜单包、执行点引用的权限；
- 发布时校验菜单仅绑定 FUNCTION/RESOURCE，叶子菜单才有 MENU 权限。
...
认证层 Authentication
  ...
  -> 功能/资源授权层 Capability Authorization
...
`GET /api/...` 不再出现在角色、前端按钮或运营授权界面中。框架扫描注解，把接口路径和处理方法同步到 `iam_endpoint`，用于覆盖检查。
...
3. 验证其至少有 RequireFunction/RequireResource；
4. 验证注解代码在权限目录中存在且为 ACTIVE；
5. 验证高风险接口不是被过宽功能意外覆盖；
6. 输出孤儿功能、孤儿资源、已下线接口和重复绑定。
```
...
前端不需要拿到 HTTP 路径权限列表。技术资源明细保留在权限解释页或研发接口中。
...
模拟器输出最终结果、功能/资源来源、数据策略来源和对象归属，是管理员与研发排障的共同语言。
...
将 `crm.customer.export` 作为“客户列表”的 OPTIONAL 高风险功能，而不是 CORE；资源模板可随该功能或单独授权。
...
- 角色只拥有其中一部分 API 时，不能直接授予更宽功能；应拆功能或保留资源级兼容；
...
2. 前端逐模块切换到功能码；
3. 后端逐模块使用 `@RequireFunction/@RequireResource`；
...
- 权限上下文只向前端返回必要菜单和功能码，不返回完整接口注册表。
...
以下资料用于校准行业做法和安全边界；本方案根据“小型企业、单一中台、无多租户”的目标做了主动收敛。
