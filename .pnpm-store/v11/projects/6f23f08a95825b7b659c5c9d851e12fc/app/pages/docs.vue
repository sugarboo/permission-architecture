  ...
  <div>
    ...
    <div class="docs-layout">
      <aside class="panel docs-nav">
        <div class="panel-head"><div><h3>本页目录</h3></div></div>
        <div class="panel-body nav-list">
          <a v-for="item in sections" :key="item.id" :href="`#${item.id}`" class="nav-link" style="color: #526178"><UIcon name="i-lucide-hash" size="13" />{{ item.label }}</a>
        </div>
      </aside>
...
允许(u, resourceCode)
  = 用户有效 AND resourceCode ∈ 有效资源(u)</pre>
        ...
        <h2 id="flow">授权计算</h2>
        <h3>角色优先，直授补充</h3>
        <p>用户授权抽屉首先展示可多选的角色模板，再配置菜单和少量直接资源。勾选任一角色时，其菜单立即以只读勾选反显；对应 CORE 资源自动勾选，OPTIONAL 资源统一黄色高亮，并区分“角色已授”和“待选”。这些投影只用于预览，保存时仍分别记录角色和用户直授，避免把继承权限复制成直授。</p>
        <h3>菜单资源包</h3>
