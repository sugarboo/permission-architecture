...
type Catalog = {
  resources: Array<{ id: string, code: string, name: string, domain: string, riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' }>
  menuTree: MenuNode[]
}

function groupPermissions(items: Catalog['resources']): PermissionTreeNode[] {
...
export function usePermissionCatalog(catalog: Ref<Catalog | null>) {
  ...
  const resourceTree = computed(() => catalog.value ? groupPermissions(catalog.value.resources) : [])
  return { menuTree, resourceTree }
}
