...
Application 和 Menu 已有独立领域，不再重复注册为 Resource。

HTTP API 也不作为权限 Resource。

## 8.2 业务对象资源

```text
information.company      企业信息
supply.supplier          供应商
supply.purchase_order    采购单
designer.profile         设计师档案
finance.payable          应付单
hr.employee              员工档案
```
...
字段 Resource 作为 BUSINESS_OBJECT 的子资源：

```text
supply.supplier.name
supply.supplier.contact_mobile
supply.supplier.bank_account
supply.supplier.purchase_amount
```
...
```text
错误：为供应商 SUP-001 创建 auth_resource
正确：只注册 supply.supplier，SUP-001 在请求运行时作为 resourceId
```
...
Function 表示“用户可以对某个 Resource 执行什么动作”。

```text
Function = Resource + Action + Stable Permission Code
```
...
| 资源   | 动作    | 数据范围       |
| ------ | ------- | -------------- |
| 供应商 | read    | DEPT_AND_CHILD |
| 供应商 | update  | DEPT_AND_CHILD |
| 供应商 | export  | DEPT_AND_CHILD |
| 供应商 | approve | ALL            |
...
| Menu        | 用户从哪里进入？             | 供应商列表           |
| Resource    | 正在操作什么对象？           | 供应商               |
| Function    | 可以对对象做什么？           | 编辑供应商           |
| DataPolicy  | 可以操作哪些记录？           | 本人及协作           |
| FieldPolicy | 记录中的哪些内容可以怎么用？ | 手机号脱敏且禁止导出 |
...
对 supply.supplier 资源执行 read
数据范围为 SELF_AND_SHARED
其中 contact_mobile 字段 MASKED
```
...
action_code
policy_id FK
UNIQUE(role_id, resource_id, action_code)
```
...
erDiagram
    ...
    ORG_UNIT ||--o{ SYS_USER : has
    ORG_POSITION ||--o{ SYS_USER : assigned
    SYS_USER ||--o{ AUTH_USER_ROLE : has
    AUTH_ROLE ||--o{ AUTH_USER_ROLE : assigned

    AUTH_APPLICATION ||--o{ AUTH_MODULE : contains
    AUTH_APPLICATION ||--o{ AUTH_MENU : contains
    AUTH_APPLICATION ||--o{ AUTH_RESOURCE : owns
    AUTH_APPLICATION ||--o{ AUTH_FUNCTION : owns

    AUTH_MODULE ||--o{ AUTH_MENU : groups
    AUTH_MODULE ||--o{ AUTH_RESOURCE : groups
    AUTH_MODULE ||--o{ AUTH_FUNCTION : groups

    AUTH_MENU ||--o{ AUTH_MENU : children
    AUTH_FUNCTION ||--o{ AUTH_MENU : minimum_entry

    AUTH_RESOURCE ||--o{ AUTH_RESOURCE : children
    AUTH_RESOURCE ||--o{ AUTH_FUNCTION : target
    ...
    AUTH_ROLE ||--o{ AUTH_ROLE_DATA_POLICY : scoped
    AUTH_RESOURCE ||--o{ AUTH_ROLE_DATA_POLICY : target
    AUTH_DATA_POLICY ||--o{ AUTH_ROLE_DATA_POLICY : policy

    AUTH_ROLE ||--o{ AUTH_ROLE_FIELD_POLICY : field_rule
    AUTH_RESOURCE ||--o{ AUTH_ROLE_FIELD_POLICY : field
...
```java
@RequirePermission("supply.supplier.read")
@DataScope(resource = "supply.supplier", action = "read")
@GetMapping("/suppliers")
public Page<SupplierVO> page(...) {
    ...
}
...
```java
@RequirePermission("supply.supplier.update")
@DataScope(resource = "supply.supplier", action = "update")
@FieldPermission(resource = "supply.supplier", action = "update")
...
| 后端开发        | Function 定义、资源绑定、强制鉴权、DataPolicy、FieldPolicy、审计 | 不以按钮是否隐藏作为授权依据               |
| 系统权限管理员  | 创建角色、配置菜单/功能/数据/字段权限、分配用户、审计授权    | 不修改代码，不自行编写 SQL 权限条件        |
...
@DataScope(resource = "supply.supplier", action = "export")
@FieldPermission(resource = "supply.supplier", action = "export")
...
2. 当前角色对 supply.supplier 的 export 数据范围是什么
3. 查询是否只包含允许的数据记录
4. 哪些 FIELD 可以进入导出文件
5. 是否需要记录高风险导出审计
```
...
### 后端定义 BUSINESS_OBJECT

```json
{
  "resourceCode": "supply.purchase_contract",
  "resourceName": "采购合同",
  "resourceType": "BUSINESS_OBJECT",
  "ownerField": "owner_user_id",
  "deptField": "owner_dept_id",
  "creatorField": "created_by",
  "collaboratorRelation": "purchase_contract_collaborator"
}
```

这一步决定未来 DataPolicy 如何识别记录归属。

### 注册业务 Function

```text
supply.purchase_contract.read
supply.purchase_contract.create
supply.purchase_contract.update
supply.purchase_contract.delete
supply.purchase_contract.export
supply.purchase_contract.approve
```

### 前端使用相同 Function Code

```vue
<Can permission="supply.purchase_contract.create">
...
```text
read   → SELF_AND_SHARED
create → Function ALLOW
update → SELF
export → NONE
approve → NONE
```
...
→ Action
→ RoleDataPolicy
→ Record owner / dept / collaborator
```
...
```text
functionCode + resource + action
```

权限管理员看到：

```text
functionName + 所属业务对象 + 风险等级
```
...
Module      负责业务分组
Menu        负责导航与页面入口
Resource    负责描述被保护的业务对象
Function    是稳定的业务权限编码
Role        是可复用权限集合
User        直接等同于企业员工
DataPolicy  决定可访问哪些记录
FieldPolicy 决定记录内部字段如何使用
```
...
该结构保留了 RBAC 的易理解和易维护，同时通过组织数据范围与字段策略覆盖 CRM、供应链、财务、HR 等企业系统常见的精细化授权需求；又避免了 Account / Employee 分层、角色继承、通用 DENY、任意 ABAC、API-Based 权限等第一阶段不必要的复杂度。
